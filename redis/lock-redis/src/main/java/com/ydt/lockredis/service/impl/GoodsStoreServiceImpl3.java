...
import com.ydt.lockredis.util.RedisWaitLock;
 ...
 *
 */
@Service("goodsStoreServiceImpl3")
public class GoodsStoreServiceImpl3 implements GoodsStoreService {

	@Autowired
	private RedisWaitLock redisWaitLock;
	...
	public String updateGoodsStore(String code,int count) {
		...
		if(!redisWaitLock.lock(code,String.valueOf(time))){
			return "排队人数太多，请稍后";
		}
		...
		try {
			...
			//释放锁
			redisWaitLock.release(code,String.valueOf(time));
